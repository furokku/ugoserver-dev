...
func (e *env) load_html(reload bool) error {
    ...
    // html part
    h, err := template.ParseGlob(fmt.Sprintf("%s/%s/special/html/*.html", e.cnf.Dir, assets_dir))
    if err != nil {
        return err
    }
...
func (e *env) load_assets(reload bool) error {
    ...
    fs.WalkDir(os.DirFS(e.cnf.Dir + "/" + assets_dir), ".", walktomap(&a))
...
func (e *env) load_menus(reload bool) error {
    ...
    m := make(map[string]Ugomenu)
    fs.WalkDir(os.DirFS(e.cnf.Dir), assets_dir + "/special/menu", func(path string, d fs.DirEntry, err error) error {
        if err != nil {
...
func walktomap(m *map[string][]byte) (fs.WalkDirFunc) {
    return func(path string, d fs.DirEntry, err error)(error){
        ...
        fc, err := os.ReadFile(assets_dir + "/" + path)
        if err != nil {
