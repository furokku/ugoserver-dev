...
const (
    ...
    // Size of blank ppm
    // This should be a sane default, anything more could
    // case good ppms to be ignored
    MIN_PPM_SIZE = 0x7C4
)
...
func (e *env) verifyrsa(ppm []byte) error {
    if len(ppm) < MIN_PPM_SIZE {
        return ErrInvalidPpmLength
    }
    edge := len(ppm)
    
    h := sha1.Sum(ppm[ : edge-0x90])
    
    return rsa.VerifyPKCS1v15(e.fnkey, crypto.SHA1, h[:], ppm[edge-0x90 : edge-0x10])
}